...
Le projet d'IF36 est pour nous l'occasion d'étudier un domaine qui nous intéresse. Etant nous-mêmes étudiants, quoi de mieux que les conditions de vie des étudiants? Notre objectif durant cette analyse exploratoire d'un dataset sur ce sujet sera de mettre en avant l'existence ou non de relations entre différents aspects de la vie d'étudiants. Nous couvrirons globalement l'ensemble des pilliers sur lesquels repose la vie typique d'un étudiant: la scolarité, l'éducation, la vie extra-scolaire et les relations privées.

### Données

Notre dataset provient de l'UCI Machine Learning trouvé sur kaggle.
...
Information sur le dataset de référence : P. Cortez and A. Silva. Using Data Mining to Predict Secondary School Student Performance. In A. Brito and J. Teixeira Eds., Proceedings of 5th FUture BUsiness TEChnology Conference (FUBUTEC 2008) pp. 5-12, Porto, Portugal, April, 2008, EUROSIS, ISBN 978-9077381-39-7.
...
Pour commencer notre étude exploratoire des habitudes de vie des étudiants nous souhaitions analyser une possible influence des sorties entre amis sur la consommation d'alcool.
...
## Question 2 - les sportifs ont-ils de meilleurs résultats scolaires que les autres ?
...
Nous tenterons maintenant d'analyser les différents résultats obtenus jusqu'à présent.
...
```{r Question 8 : Distance correlée absentéisme bar, fig.align='center', echo=FALSE}
#Question 8 :
...
distance_abs_dataset_mat <- aggregate(absences ~ traveltime, data = tableMat, FUN = mean)
distance_abs_dataset_por <- aggregate(absences ~ traveltime, data = tablePor, FUN = mean)
...
```{r Question 8 : Distance correlée absentéisme violin, fig.align='center', echo=FALSE}

#Etablir les deux datasets avec le nombre d'absences et le temps de trajet
distance_abs_dataset_mat2 <- tableMat[, c("absences", "traveltime")]
distance_abs_dataset_por2 <- tablePor[, c("absences", "traveltime")]
...
```{r Question 8 : Distance correlée absentéisme boxplot, fig.align='center', echo=FALSE}
...
```{r Question 8 : Distance correlée absentéisme maths, fig.align='center', echo=FALSE}
...
```{r Question 8 : Distance correlée absentéisme portugais, fig.align='center', echo=FALSE}
#Question 8 :
#La temps de trajet jusque son lieu d'étude est-il un facteur d'absence ?

distance_abs_dataset_mat3 <- tableMat
distance_abs_dataset_por3 <- tablePor
...
```{r Question 8 : Absences Potrugais, fig.align='center', echo=FALSE}

#En fonction du studytime ?

#Etablir les deux datasets avec le nombre d'absences et le temps de trajet
studytime_abs_dataset_mat <- aggregate(absences ~ studytime, data = tableMat, FUN = mean)
studytime_abs_dataset_por <- aggregate(absences ~ studytime, data = tablePor, FUN = mean)
...
```{r Question 8 : Nombre eleves Portugais, fig.align='center', echo=FALSE}

#Etablir les deux datasets avec le nombre d'absences et le temps de trajet
studytime_abs_dataset_mat2 <- tableMat[, c("absences", "studytime")]
studytime_abs_dataset_por2 <- tablePor[, c("absences", "studytime")]
...
```{r Question 8 :Absence Maths, fig.align='center', echo=FALSE}

#En fonction du studytime ?

#Etablir les deux datasets avec le nombre d'absences et le temps de trajet
studytime_abs_dataset_mat3 <- aggregate(absences ~ studytime, data = tableMat, FUN = median)
studytime_abs_dataset_por3 <- aggregate(absences ~ studytime, data = tablePor, FUN = median)
...
En démarrant notre analyse exploratoire du dataset nous voulions savoir si les données pouvaient nous permettre de détruire ou confirmer des stéréotypes, découvrir des tendances comportementales chez les étudiants et apporter de potentielles pistes de réflexion sur la psychologie de cette population. Puisque nous n'avions pas pour but de prouver un phénomène mais plutôt d'en découvrir, nous avons dû bien prendre le temps de comprendre notre dataset et réfléchir aux manières de lier les différentes features à notre disposition. De cette réflexion sont nées les huit questions sur lesquelles nous nous sommes penchés.

L'analyse exploratoire de notre dataset nous a permis de dégager plusieurs constats significatifs concernant les comportements et les performances scolaires des individus en fonction de divers facteurs. Voici les conclusions principales de notre étude :

### Consommation d'alcool et âge :
Nous avons observé une différence notable dans la consommation d'alcool en fonction de l'âge des individus sortant fréquemment entre amis. Les mineurs semblent plus influençables et tendent à imiter les comportements des plus âgés, ce qui entraîne une consommation d'alcool de plus en plus précoce.

### Impact des activités extra-scolaires sur les études :
Il est bénéfique pour les études de pratiquer des activités extra-scolaires, indépendamment de l'état de santé de l'individu. Cependant, il est crucial d'adapter ces activités au niveau de santé de chaque individu pour maximiser les résultats scolaires.

### Performance en mathématiques et appartenance à la communauté geek :
Contrairement à une croyance populaire, notre analyse montre que les geeks n'obtiennent pas de meilleures notes en mathématiques que les non-geeks.

### Consommation d'alcool et redoublement :
Il existe une corrélation proportionnelle entre la consommation d'alcool et le taux de redoublement. Une consommation d'alcool plus élevée est associée à une probabilité accrue de redoublement scolaire.

### Influence du niveau d'études des parents :
Le niveau d'études des parents a un impact réel et significatif sur les résultats scolaires de leurs enfants. Les enfants de parents ayant un niveau d'études plus élevé tendent à obtenir de meilleurs résultats académiques.

### Origine géographique et aspirations éducatives :
Les étudiants ayant grandi en milieu rural montrent une tendance à s'ouvrir aux opportunités éducatives en dehors de leur région d'origine. Ils préfèrent souvent quitter leur environnement familial pour obtenir une meilleure éducation.

### Assiduité et performance scolaire :
Les élèves qui sont régulièrement présents en cours, sérieux et assidus, montrent de meilleures performances scolaires. En revanche, les élèves souvent absents ne compensent pas leur retard par un travail accru en dehors des cours, ce qui affecte négativement leurs résultats.

Ces observations fournissent des informations précieuses pour comprendre les divers facteurs influençant les comportements et les performances scolaires. Elles soulignent l'importance de l'environnement familial, de la gestion des activités extra-scolaires, de l'assiduité en classe, et des comportements sociaux sur le parcours éducatif des étudiants. 

Nous sommes d'ailleurs persuadés que si un tel dataset pouvait être réalisé à une échelle plus importante (régionale, nationale, mondiale) le résultat serait précieux et permettrait des avancées sur la compréhension des étudiants, de leurs modes de vie et de leurs besoins. En effet, les résultats de notre analyse sont intéressants mais ne peuvent pas être considéré comme une généralité du fait du nombre de réponses prélevées et de la provenance commune des étudiants.Cependant, ils peuvent servir de base à des interventions ciblées visant à améliorer les résultats scolaires et à soutenir les étudiants dans leur développement académique et personnel.

Enfin, ce projet nous a permis de réaliser une première analyse exploratoire poussée d'un dataset pour l'ensemble des membres du groupe. Nous avons également dû travailler en groupe, collaborer et nous organiser de manière réfléchie et selon les capacités de chacun. L'utilisation de Git nous a permis de versionner notre travail et de profiter d'une version centralisée du projet modifiable par chacun en local avant une mise en commun.


## Commentaires de chaque membre du groupe

### Théo: 
Ce projet m'a permis d'aquérir de nouvelles compétences techniques mais aussi organisationnelles et relationnelles dans la gestion de projets orientés Data. Je pense qu'il aurait pû être pertinent de développer les manières de partager une analyse exploratoire d'un dataset afin d'en faire un réel support de réflexion et imaginer les moyens de présenter des résultats au plus grand nombre.

### Louis: 
La réalisation de ce projet m'a été très bénéfique et m'a apporté des compétences à la fois techniques : coder en R, faire des dataviz parlantes, meilleure utilisation de git, savoir exploiter des données et avoir un esprit critique dessus ... et aussi organisationnelles grâce à la gestion que l'on a pu avoir du projet tout au long du semestre. Il aurait été intéressant pour ce projet d'avoir un dataset encore plus complet afin de répondre à d'autres interrogations sur lesquelles nous ne pouvions pas avoir de réponse ici. 
Également, développer un shiny dashboard traitant l'ensemble des questions que nous avons abordées aurait été long, mais pertinent car permettrait une plus facile exploitation des résultats obtenus et faciliterait un partage et une l'utilisation facilitée et plus ouverte à un grand public.


### Lucas:
Projet complet et formateur sur les différentes notions de dataviz mais aussi sur la gestion de projet (git / projet à 4 personnes). Cela donne envie de s'investir dans des datasets beaucoup plus larges où l'on pourraient faire des liens de corrélation plus détaillée. 


### Matthieu:
Ce projet fut pour moi l'occasion d'apprendre à mettre en œuvre la théorie vue en cours sur la visualisation de données, dans le but de créer des visualisations adéquates à l'analyse de nos données. Il aurait été intéressant d'avoir plus de données continues, afin de pouvoir réaliser d'autres types de visualisations et ainsi d'avoir plus de données intéressantes.   


## Annexe

Afin de se partager au mieux le travail et d'avoir un travail de groupe efficace, nous avons décidé de nous attribuer chacun plusieurs questions. Voici notre répartition du travail :
...
Question 2 : Les sportifs ont-ils de meilleurs résultats scolaires que les autres ?
Question 8 : Existe-t-il une corrélation entre le temps de trajet entre le domicile et l'université et l'absentéisme chez les étudiants ?
Shiny Dashboard de la question de Théo : Question 1: Le fait de sortir souvent avec des amis entraîne-t-il nécessairement une plus grande consommation d'alcool ?
