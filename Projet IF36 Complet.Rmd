...
Nous allons dans un premier temps, essayer de visualiser le nombre d'absences moyen par temps de trajet.
Pour ceci, il est nécessaire de récupérer les informations dont on a besoin dans nos datasets student_mat et student_por.
Ici, on extrait directement le nombre d'absences moyen par catégorie de temps de trajet, et on visualise.
...
Ces résultats semblent illogique : plus le temps de trajet est long, plus l'absentéisme est un gain de temps pour un étudiant. 
De plus, avec plus de temps de trajet, les problèmes de transport sont plus courants et donc retards et absentéismes devraient l'être aussi.
...
En appliquant ces améliorations, on peut obtenir ceci.
...
```
En appliquant ces modifications, on identifie clairement que le temps de trajet ne semble pas avoir d'influence sur le nombre d'absences.
...
On se pose ainsi la question suivante : les étudiants travaillant plus en dehors des cours, sont-ils plus ou moins absents ?

En effet la question fait sens car deux possibilités semblent pouvoir être vraies.
...
Pour ceci, nous prenons la même méthodologie que précedemment mais nous changeons la variable travel_time avec studytime. Regardons le nombre d'absences moyen en fonction du temps d'étude.
```{r Question 5 : Distance correlée absentéisme, fig.align='center', echo=FALSE}
